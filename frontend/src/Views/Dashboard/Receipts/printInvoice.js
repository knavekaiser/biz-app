...
const PrintInvoice = forwardRef(({ receipt, user }, ref) => {
    ...
    <div className={s.print} ref={ref}>
      ...
      <div className={`${s.info} flex wrap gap-1 mt-1`}>
        <div className={`${s.box} flex-1`}>
          <p>To:</p>
          <Detail label="Name" value={receipt.customer?.name} />
          <Detail
            label="Detail"
            value={
              receipt.customer?.detail?.split("\n").map((line, i, arr) => (
                <span key={i}>
                  {line}
                  {arr[i + 1] && <br />}
                </span>
        ...
        <div className={`${s.box} flex-1`}>
          <Detail
            label="Inv No"
            value={`${receipt.no}${config.print?.receiptNoSuffix || ""}`}
          />
          <Detail label="Date" value={moment(receipt?.date, "DD-MM-YYYY")} />
      ...
      <div className={`${s.totalAmount} mt-1`}>
        <p className={s.word}>
          Total:{" "}
          {(
            receipt.items.reduce((p, c) => p + c.qty * c.price, 0) +
            receipt.items
              .reduce((p, c) => p + c.qty * c.price, 0)
              .percent(receipt.gst || 0)
          ).toWords()}
        </p>
        <p className={s.digit}>
          {(
