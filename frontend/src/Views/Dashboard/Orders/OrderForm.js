...
const Form = ({ edit, orders, onSuccess }) => {
  ...
  return (
    <div
      className={`grid gap-1 ${s.addOrderForm} ${viewOnly ? s.viewOnly : ""}`}
    >
      ...
      {viewOnly && (
        <div className="flex gap-1 all-columns border-t p-1 align-center">
          {checkPermission("order_update") && (
            <button className="btn medium" onClick={() => setViewOnly(false)}>
              Edit
            </button>
          )}
  ...
  return (
    ...
    >
      ...
      <div className="btns">
        ...
        <button className="btn medium" disabled={disabled || loading}>
          {edit ? "Update" : "Submit"}
        </button>
      </div>
    </form>
